...
module.exports.dispatcher = {
  ...
  rootRouter: function (router) {
    /**
     * 将xxx.bdbvip.com:8085/ 转发成 bdbvip.com:8085/xxx/
    ...
    router.use('/', function (req, res, next) {
      if (req.path.split('/')[1] == '_common') {
        ...
        /*var secondHostname = '';
         if (req.hostname == 'localhost' || /\d+\.\d+\.\d+\.\d+/.test(req.hostname)) {
         secondHostname = 'cus';
         } else {
         secondHostname = req.hostname.split('.')[0];
         }*/
        var secondHostname = req.hostname.split('.')[0];
