module.exports = {
	...
	getCardTypeName: function (cardType) {
		var root = this;
		if (root.cardTypeMap[cardType]) {
			return root.cardTypeMap[cardType]
		}
		return '';
	},
	getBankNameByBankCode: function (bankcode) {
		var root = this;
		for (var i = 0, len = root.bankcardList.length; i < len; i++) {
			var bankcard = root.bankcardList[i];
		...
		for (var i = 0, len = root.bankcardList.length; i < len; i++) {
			...
			for (var j = 0, jLen = patterns.length; j < jLen; j++) {
				...
				if ((new RegExp(pattern.reg)).test(cardNo)) {
					var info = root.extend(bankcard, pattern);
					delete info.patterns;
					delete info.reg;
					info['cardTypeName'] = root.getCardTypeName(info['cardType']);
					return info;
				}
			}
		}
